    ...
    <>
      ...
      <Section tone="cream" className="!pt-8 lg:!pt-10">
        <nav aria-label="Synergiefelder" className="flex flex-wrap gap-2.5 pb-10">
          {synergyFields.map((f) => (
            <a
              key={f.slug}
              href={`#${f.slug}`}
              className="flex items-center gap-2 rounded-full border border-line bg-white px-4 py-2 font-sans text-sm font-medium text-ink/75 transition-colors hover:border-gold-deep hover:text-forest"
            >
        ...
        <div className="relative">
          <RiverThread className="pointer-events-none absolute -inset-x-10 -top-10 -bottom-10 hidden w-[calc(100%+5rem)] opacity-[0.35] lg:block" tone="forest" />
